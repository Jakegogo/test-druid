...
package org.test.test_druid.firewall.ratelimiter;
...
import org.test.test_druid.firewall.ratelimiter.SmoothRateLimiter.SmoothBursty;
import org.test.test_druid.firewall.ratelimiter.SmoothRateLimiter.SmoothWarmingUp;
...
public abstract class RateLimiter {
  ...
  /**
   * 判断能否获得令牌(fast fail)
   * <br/> 此方法不会阻塞调用线程, 如果返回true, 需再次调用acquire(..)或tryAcquire(..)方法获取令牌
   * <br/> 如果返回false, 则是获取令牌失败
   * @author jake
   * @param permits the number of permits to acquire
   * @param timeout the maximum time to wait for the permits. Negative values are treated as zero.
   * @param unit the time unit of the timeout argument
  ...
  /**
	 ...
	 * @return
	 */
	public abstract double getUsedPermits();
