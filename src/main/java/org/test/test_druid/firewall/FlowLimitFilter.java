 ...
 */
public class FlowLimitFilter extends FilterEventAdapter implements
	...
	private void acquireToken(StatementProxy statement) {
		...
		if (!userCounter.acquire()) {
			logger(FlowLimitLogDto.valueOf(DataSourceLogType.Limit_Overflow,
					...
					userCounter.getCurCount(), userCounter.getUpperLimit(),
					userCounter.getName()));
			userCounter.rethrowTimeout();
		} else if (userCounter.hasWarn()) {// 输出警告
			logger(FlowLimitLogDto.valueOf(DataSourceLogType.Limit_Warn,
					statement.getConnectionProxy().getId(), enterpriseNum, userId, null,
					userCounter.getCurCount(), userCounter.getLimit(),
					userCounter.getName()));
		}
		...
		if (!enterpriseCounter.acquire()) {
			...
			enterpriseCounter.rethrowTimeout();
	 ...
	 * 用户限流
	 */
	private LimitCounter userCounter = LimitCounterBuilder.createRateLimiter("用户限流").build();
